...
const Update = () => {
  ...
  const users = useSelector((state) => state.users); //get the users from the store
  const existingUser = users.find((user) => user.id === parseInt(id)); //find the user with the id
  ...
  const onSubmitHandler = (e) => {
    e.preventDefault();
    dispatch(updateUser({ id: existingUser.id, name, email }));
    navigate("/");
  };
        ...
        <div className="relative flex flex-col rounded-xl bg-transparent bg-clip-border text-gray-700 shadow-none">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-5">
              <button className="mt-6 block  select-none rounded-full bg-red-800 py-3 px-6 text-center align-middle  uppercase text-white shadow-md shadow-green-800/20 transition-all hover:shadow-lg hover:shadow-green-800/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                <a href="/">Go Back</a>
              </button>
              <button
                data-testid="registerBtn"
                className="mt-6 block  select-none rounded-full bg-green-800 py-3 px-6 text-center align-middle  uppercase text-white shadow-md shadow-green-800/20 transition-all hover:shadow-lg hover:shadow-green-800/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                type="submit"
